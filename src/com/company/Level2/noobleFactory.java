...
public class noobleFactory {
    //heap = 우선순위 큐
    //먼저 우선순위 큐는 항상 정렬된 자료구조로서
    //큐에서 poll()하는 값은 항상 정렬된 가장 첫번째 요소가 나오게 된다.
    public int solution(int stock, int[] dates, int[] supplies, int k) {
        int answer = 0;
        int index=0;
        PriorityQueue<Integer> pq = new PriorityQueue<Integer>(Comparator.reverseOrder());
        // i = 현재날짜 (0일부터 시작)
        // dates = 해외에서 배송해 줄 수 있는 날짜
        for(int i=0;i<k;i++){
            if(index < dates.length && i == dates[index]){
                pq.add(supplies[index]);
                index++;
            }
            //재고가 없어지면 가장 많은 밀가루 양을 공급받는다.
            //밀가루가 필요한 시점에, 공급받을 수 있는 밀가루중에서 가장 큰 밀가루를 공급받는다.
            if(stock==0){
                stock += pq.poll();
                answer++;
            }
